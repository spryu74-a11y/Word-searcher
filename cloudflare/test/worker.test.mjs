...
test("rejects same-site or subresource API requests before any upstream call", async () => {
  for (const headers of [
    ...
    let upstreamCalled = false;
    const response = await handleRequest(
      new Request("https://edge.example/api/opendict/search?q=%EA%B0%92", { headers }),
      {},
      {},
      { fetchImpl: async () => { upstreamCalled = true; } }
    );
